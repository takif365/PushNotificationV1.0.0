const admin = require('firebase-admin');

// Hardcoded credentials for this test script ONLY
const serviceAccount = {
    projectId: "push-notification-d1fb7",
    clientEmail: "firebase-adminsdk-fbsvc@push-notification-d1fb7.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\nwYdcxWVU1vs9HAV0r5yl6RLB\n-----END PRIVATE KEY-----\n"
};

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert(serviceAccount)
    });
}

const db = admin.firestore();

async function inspectTokens() {
    try {
        console.log('🔍 Inspecting ALL tokens in DB...');
        const tokensSnap = await db.collection('push_tokens').get();

        if (tokensSnap.empty) {
            console.error('❌ No tokens found in the ENTIRE database!');
            return;
        }

        console.log(`✅ Found total ${tokensSnap.size} tokens:`);

        tokensSnap.forEach(doc => {
            const data = doc.data();
            console.log(`ID: ${doc.id} | DomainID: ${data.domainId} | Platform: ${data.platform} | UserAgent: ${data.userAgent?.substring(0, 50)}...`);
        });

    } catch (error) {
        console.error('Script error:', error);
    }
}

inspectTokens();
